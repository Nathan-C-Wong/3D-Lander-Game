...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (Box& box : boxList) {
		vector<int> pointLst;
		if (getMeshPointsInBox(mesh, node.points, box, pointLst) > 1) {
			TreeNode child;
			child.box = box;
			child.points = pointLst;
...
// Ray node-box intersection
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	//bool intersects = false;

	if (!node.box.intersect(ray, 0.1f, 10000.0f)) {
		return false;
	}
...
void Octree::draw(TreeNode & node, int numLevels, int level, vector<ofColor>& levelColours) {
	if (level >= numLevels) return;

	ofSetColor(levelColours[level % levelColours.size()]);

	drawBox(node.box);
	level++;
	for (int i = 0; i < node.children.size(); i++) {
		draw(node.children[i], numLevels, level, levelColours);
	}
}
